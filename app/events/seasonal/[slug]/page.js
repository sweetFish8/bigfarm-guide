  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <nav className="text-sm text-stone-500 dark:text-stone-400 mb-6">
        ...
        <span className="mx-2">/</span>
        <Link href="/events/seasonal" className="hover:text-green-700 dark:text-green-400">季節限定マップ</Link>
        <span className="mx-2">/</span>
        <span>{s.name}</span>
      </nav>
      ...
      {s.overview && s.overview.length > 0 && (
        <section className="bg-white dark:bg-stone-900 rounded-2xl p-6 border border-stone-200 dark:border-stone-700 mb-6">
          <h2 className="text-xl font-bold text-green-800 dark:text-green-300 mb-3">📋 イベント概要</h2>
          <ul className="space-y-2 text-stone-700 dark:text-stone-200 text-sm leading-relaxed">
            {s.overview.map((o, i) => (
      ...
      {Object.keys(grouped).length > 0 && (
        <section className="bg-amber-50 dark:bg-stone-900 border border-amber-200 dark:border-amber-900 rounded-2xl p-6 mb-6">
          <h2 className="text-xl font-bold text-green-800 dark:text-green-300 mb-4">🗂 固有アイテム</h2>
          <div className="space-y-5">
            {Object.entries(grouped).map(([type, items]) => (
              <div key={type}>
                <h3 className="text-sm font-bold text-amber-800 dark:text-amber-300 uppercase tracking-wide mb-2">
                  {type}
                </h3>
                <ul className="grid sm:grid-cols-2 gap-3">
                  {items.map((it, i) => (
                    <li key={i} className="bg-white dark:bg-stone-900 rounded-lg p-3 border border-amber-200 dark:border-amber-900">
                      <p className="font-semibold text-stone-800 dark:text-stone-100">{it.name}</p>
                      <p className="text-xs text-stone-600 dark:text-stone-300 mt-1 leading-relaxed">{it.desc}</p>
                    </li>
                  ))}
                </ul>
              </div>
      ...
      <section className="bg-green-50 dark:bg-stone-900 border border-green-200 dark:border-green-900 rounded-2xl p-6 mb-6">
        <h2 className="text-xl font-bold text-green-800 dark:text-green-300 mb-3">🎁 報酬</h2>
        <p className="text-stone-700 dark:text-stone-200">{s.rewards}</p>
      </section>

      {s.tip && (
        <section className="bg-yellow-50 dark:bg-stone-900 border border-yellow-200 dark:border-yellow-900 rounded-2xl p-6">
          <h2 className="text-xl font-bold text-green-800 dark:text-green-300 mb-3">💡 攻略のコツ</h2>
          <p className="text-stone-700 dark:text-stone-200 leading-relaxed">{s.tip}</p>
        </section>
      )}

      <div className="mt-6">
        <Link href="/events/seasonal" className="text-green-700 dark:text-green-400 font-semibold hover:underline">
          ← 季節限定マップ一覧
        </Link>
      </div>
    </div>
